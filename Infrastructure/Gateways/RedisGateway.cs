...
public class RedisGateway
{
    ...
    {
        _logger.LogServerInformation(title: "CACHE_INPUT", parameters: new { key });
        var sw = Stopwatch.StartNew();
        string dataString = await _db.StringGetAsync(key);
        var data = dataString.FromJson<T>();
        _logger.LogServerInformation(sw: sw, title: "CACHE_OUTPUT", parameters: new { data });
        return data;
    }

    public async Task SetData<T>(string key, T data, TimeSpan? lifeSpan = null)
    {
        _logger.LogServerInformation(title: "CACHE_INPUT", parameters: new { key, lifeSpan, data });
        var sw = Stopwatch.StartNew();
        var dataString = data.ToJson(ignoreSensitiveLimit: true);
        await _db.StringSetAsync(key, dataString, lifeSpan);
        _logger.LogServerInformation(sw: sw, title: "CACHE_OUTPUT");
    }
    ...
    {
        _logger.LogServerInformation(title: "CACHE_INPUT", parameters: new { key, lifeSpan });
        var sw = Stopwatch.StartNew();
